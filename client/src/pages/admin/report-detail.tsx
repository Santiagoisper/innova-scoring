...
import { fetchReport, fetchSite, fetchReportSignatures, fetchAdminUsers, acknowledgeReport } from "@/lib/api";
...
export default function ReportDetail() {
  ...
  const handleAcknowledge = () => {
    if (!user?.name || !reportHash) {
      toast({
        variant: "destructive",
        title: "Cannot acknowledge report",
        description: "Missing user session or report hash.",
      });
      return;
    }
  ...
  const handleDownloadPDF = () => {
    ...
    const drawFooter = () => {
      ...
      doc.text(
        `Generated on ${new Date(reportGeneratedAt || Date.now()).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })} | Innova Trials LLC`,
    ...
    y += 6;
    doc.text(`Site: ${site?.contactName || `Site #${reportSiteId}`}`, margin + 7, y);
    y += 6;
    doc.text(`Location: ${site?.location || "N/A"}`, margin + 7, y);
    y += 6;
    doc.text(`Generated: ${new Date(reportGeneratedAt || Date.now()).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}`, margin + 7, y);
    y += 6;
    doc.text(`Final Status: ${report.finalStatus || "N/A"}`, margin + 7, y);
    y += 6;
    doc.text(`Generated By: ${generatedByResolved}`, margin + 7, y);
    y += 14;
    ...
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...gray600);
    doc.text(`SHA-256 Hash: ${reportHash || "N/A"}`, margin + 7, y);
    y += 6;
    doc.text(`Locked: ${reportLocked ? "Yes" : "No"}`, margin + 7, y);
    y += 6;
    doc.text(`Generated By: ${generatedByResolved}`, margin + 7, y);
    ...
    const siteName = site?.contactName || `Site_${reportSiteId}`;
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
              ...
              <div>
                ...
                <p className="font-semibold" data-testid="text-site-name">{site?.contactName || `Site #${reportSiteId}`}</p>
              ...
              <div>
                ...
                <p className="font-semibold" data-testid="text-generated-date">
                  {reportGeneratedAt
                    ? new Date(reportGeneratedAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })
                    : "N/A"}
                </p>
              </div>
            </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-medium" data-testid="text-lock-status">
                  {reportLocked ? "Locked — Report is immutable" : "Unlocked — Awaiting acknowledgment"}
                </p>
              </div>
            </div>
            <Separator />
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-medium" data-testid="text-generated-by">{generatedByResolved}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <Card className="border-amber-300 bg-amber-50" data-testid="section-acknowledgment">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="bg-white rounded-lg p-3 border border-amber-200">
                ...
                <code className="text-xs font-mono" data-testid="text-ack-hash">{reportHash || "N/A"}</code>
              </div>
              <Button
